...
module.exports = function (router) {
    ...
    userRouter.post(function (req, res) {
        ...
        if(req.body.email === ''){
            res.status(500).json({ message: "ERROR: You must input a valid email", data: null });
            validPost = false;
        }
        ...
        User.find(function(err, users){
            ...
            if(emails.includes(req.body.email)){
                res.status(500).json({ message: "ERROR: That email is already in use", data: req.body.email});
                validPost = false;
            }
        });
    ...
    userIdRouter.delete(function (req, res) {
        User.findByIdAndRemove(new mongoose.Types.ObjectId(req.params.id), function(err, user){
            if(err){
                res.status(404).send({ message: "ERROR: Could Not Delete Given User", data: err });
            } else if(user === null) {
                res.status(404).json({ message: "ERROR: No Such Given User", data: null });
            } else {
